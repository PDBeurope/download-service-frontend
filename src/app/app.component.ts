...
export class AppComponent {
  ...
  getDownloadParamsCompoundPdb() {
    ...
    this.fdstype = 'pdb';
    if (this.compoundPdbAtom === 'conventional') {
      this.fdsConfig['atom_naming_scheme'] = 'conventional';
    }
    else {
      this.fdsConfig['atom_naming_scheme'] = 'alternative';
    }
    if (this.compoundPdbConformer === 'model') {
      this.fdsConfig['conformer'] = 'model';
    }
    else {
      this.fdsConfig['conformer'] = 'ideal';
    }
  }
  ...
  getDownloadParamsCompoundSdf() {
    ...
    this.fdstype = 'sdf';
    if (this.compoundSdfCombined === 'combined') {
      this.fdsConfig['combined'] = true;
    }
    else {
      this.fdsConfig['combined'] = false;
    }
    if (this.compoundSdfConformer === 'model') {
      this.fdsConfig['conformer'] = 'model';
    }
    else {
      this.fdsConfig['conformer'] = 'ideal';
  ...
  radioCompoundPdbChange() {
    if (!(this.compoundPdbAtom)) {
      this.compoundPdbAtom = "conventional";
    }
    if (!(this.compoundPdbConformer)) {
      this.compoundPdbConformer = "model"
    }
    this.resetEntry();
    this.resetCompoundMmcifformat();
    this.resetCompoundSdfformat();
  ...
  radioCompoundSdfChange() {
    if (!(this.compoundSdfConformer)) {
      this.compoundSdfConformer = "model";
    }
    if (!(this.compoundSdfCombined)) {
      this.compoundSdfCombined = "combined";
    }
    this.resetEntry();
    this.resetCompoundMmcifformat();
    this.resetCompoundPDBformat();
